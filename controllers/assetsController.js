...
const generateCodes = async (req, res) => {
  ...
  const barcodeText =
    typeof rawBarcodeText === 'string' ? rawBarcodeText.trim() : ''
  const parsedLogoScale =
    qrLogoScale !== undefined && qrLogoScale !== null
      ? Number(qrLogoScale)
      : null
...
const create = async (req, res) => {
  try {
    ...
    const withUrls = {
      ...asset,
      barcode_url: asset.barcode
        ? `${protocol}://${host}${asset.barcode}`
        : null,
      qr_code_url: asset.qr_code
        ? `${protocol}://${host}${asset.qr_code}`
        : null,
      codesheet_url: asset.codesheet_path
        ? `${protocol}://${host}${asset.codesheet_path}`
        : null,
    }
...
const listByCreator = async (req, res) => {
  ...
  try {
    ...
    const result = await assetService.getAssetsByCreator(userId, pagingQuery, {
      startDate,
      endDate,
    })
